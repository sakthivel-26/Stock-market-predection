"""
Auto Stock Alert — Runs daily via Windows Task Scheduler.
Scans all stocks, finds high-conviction picks (|score| >= threshold),
and sends alerts to Telegram automatically.
"""

import os
import sys
import logging
from datetime import datetime

import yfinance as yf
import pandas as pd
import numpy as np
import ta
import joblib
import requests
from sklearn.ensemble import RandomForestClassifier

# ========================
# CONFIG — Edit these
# ========================
TELEGRAM_BOT_TOKEN = "8765047819:AAFbik0t5zY7r0XMLvtL4xCdc9OMGQD3LM0"
TELEGRAM_CHAT_ID = "5085894525"
SCORE_THRESHOLD = 6      # Alert when |score| >= this
SCAN_PERIOD = "3mo"      # Analysis period

# ========================
# SETUP
# ========================
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_PATH = os.path.join(BASE_DIR, "stock_model.pkl")
LOG_PATH = os.path.join(BASE_DIR, "auto_alert.log")

logging.basicConfig(
    filename=LOG_PATH,
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
)
log = logging.getLogger(__name__)

SCANNER_STOCKS = [
    "RELIANCE.NS", "TCS.NS", "INFY.NS", "HDFCBANK.NS", "ICICIBANK.NS",
    "SBIN.NS", "LT.NS", "WIPRO.NS", "BHARTIARTL.NS", "ITC.NS",
    "KOTAKBANK.NS", "HINDUNILVR.NS", "AXISBANK.NS", "MARUTI.NS",
    "SUNPHARMA.NS", "TATAMOTORS.NS", "NTPC.NS", "TITAN.NS",
    "BAJFINANCE.NS", "ASIANPAINT.NS", "HCLTECH.NS", "ULTRACEMCO.NS",
    "POWERGRID.NS", "ONGC.NS", "NESTLEIND.NS", "TATASTEEL.NS",
    "JSWSTEEL.NS", "M&M.NS", "ADANIENT.NS", "ADANIPORTS.NS",
    "COALINDIA.NS", "TECHM.NS", "INDUSINDBK.NS", "HINDALCO.NS",
    "DRREDDY.NS", "CIPLA.NS", "BAJAJFINSV.NS", "DIVISLAB.NS",
    "BRITANNIA.NS", "EICHERMOT.NS", "TATAPOWER.NS", "IRCTC.NS",
    "VEDL.NS", "BANKBARODA.NS", "PNB.NS", "ZOMATO.NS",
    "JIOFIN.NS", "DLF.NS", "HAL.NS", "BEL.NS",
]


def get_fetch_period(period):
    period_map = {
        "1wk": "6mo", "1mo": "6mo", "2mo": "6mo", "3mo": "6mo",
        "6mo": "6mo", "1y": "1y", "2y": "2y", "5y": "5y",
    }
    return period_map.get(period, period)


def trim_data_to_period(data, period):
    row_map = {
        "1wk": 5, "1mo": 22, "2mo": 44, "3mo": 66,
        "6mo": 132, "1y": 252, "2y": 504, "5y": 1260,
    }
    rows = row_map.get(period)
    return data.tail(rows) if rows else data


def load_model():
    if not os.path.exists(MODEL_PATH):
        return None
    try:
        m = joblib.load(MODEL_PATH)
        if hasattr(m, "n_features_in_") and m.n_features_in_ != 6:
            return None
        return m
    except Exception:
        return None


def analyze_stock(symbol, model, period):
    """Lightweight version of analyze_stock_data for alerts."""
    try:
        fetch_period = get_fetch_period(period)
        data = yf.download(symbol.upper(), period=fetch_period, progress=False)
        if data is None or data.empty:
            return None

        if isinstance(data.columns, pd.MultiIndex):
            data.columns = data.columns.droplevel(1)

        data["MA20"] = data["Close"].rolling(20).mean()
        data["MA50"] = data["Close"].rolling(50).mean()
        data["RSI"] = ta.momentum.RSIIndicator(close=data["Close"].squeeze(), window=14).rsi()

        macd_ind = ta.trend.MACD(close=data["Close"].squeeze())
        data["MACD_Hist"] = macd_ind.macd_diff()

        bb = ta.volatility.BollingerBands(close=data["Close"].squeeze(), window=20, window_dev=2)
        data["BB_Upper"] = bb.bollinger_hband()
        data["BB_Lower"] = bb.bollinger_lband()

        data["ATR"] = ta.volatility.AverageTrueRange(
            high=data["High"].squeeze(), low=data["Low"].squeeze(),
            close=data["Close"].squeeze(), window=14
        ).average_true_range()

        data["Volume_Avg20"] = data["Volume"].rolling(20).mean()
        data["Volume_Avg5"] = data["Volume"].rolling(5).mean()
        data["Return"] = data["Close"].pct_change()

        data["OBV"] = ta.volume.OnBalanceVolumeIndicator(
            close=data["Close"].squeeze(), volume=data["Volume"].squeeze()
        ).on_balance_volume()
        data["OBV_MA10"] = data["OBV"].rolling(10).mean()

        data = data.dropna()
        data = trim_data_to_period(data, period)
        if data.empty or len(data) < 2:
            return None

        latest = data.iloc[-1]
        prev = data.iloc[-2]
        price = float(latest["Close"])
        rsi_val = float(latest["RSI"])
        macd_hist = float(latest["MACD_Hist"])
        prev_macd_hist = float(prev["MACD_Hist"])
        bb_upper = float(latest["BB_Upper"])
        bb_lower = float(latest["BB_Lower"])
        atr_val = float(latest["ATR"])
        vol_cur = float(latest["Volume"])
        vol_avg20 = float(latest["Volume_Avg20"])
        vol_avg5 = float(latest["Volume_Avg5"])
        ma20_val = float(latest["MA20"])
        ma50_val = float(latest["MA50"])
        obv_rising = float(latest["OBV"]) > float(latest["OBV_MA10"])

        # Pro composite scoring (same logic as app.py)
        pro_score = 0

        ma20_dist_pct = ((price - ma20_val) / ma20_val) * 100 if ma20_val > 0 else 0
        ma50_dist_pct = ((price - ma50_val) / ma50_val) * 100 if ma50_val > 0 else 0
        in_downtrend = price < ma20_val and price < ma50_val
        in_uptrend = price > ma20_val and price > ma50_val

        # Price-to-MA distance
        if ma20_dist_pct < -5 and ma50_dist_pct < -5:
            pro_score -= 3
        elif ma20_dist_pct < -3:
            pro_score -= 2
        elif ma20_dist_pct > 5 and ma50_dist_pct > 5:
            pro_score += 3
        elif ma20_dist_pct > 3:
            pro_score += 2

        # ML model
        features = pd.DataFrame([{
            "MA20": ma20_val, "MA50": ma50_val, "RSI": rsi_val,
            "Volume": vol_cur, "Volume_Avg20": vol_avg20,
            "Return": float(latest["Return"])
        }])
        prediction = model.predict(features.values)[0]
        prob = float(model.predict_proba(features.values)[0].max())
        direction = "UP" if prediction == 1 else "DOWN"
        pro_score += 2 if direction == "UP" else -2
        if prob >= 0.7:
            pro_score += 1 if direction == "UP" else -1

        # MACD
        if prev_macd_hist <= 0 and macd_hist > 0:
            pro_score += 3
        elif prev_macd_hist >= 0 and macd_hist < 0:
            pro_score -= 3
        elif macd_hist > 0 and macd_hist > prev_macd_hist:
            pro_score += 1
        elif macd_hist < 0 and macd_hist < prev_macd_hist:
            pro_score -= 1

        # RSI (trend-aware)
        if rsi_val < 30:
            pro_score += -1 if in_downtrend else 2
        elif rsi_val > 70:
            if not in_uptrend:
                pro_score -= 2
        elif 30 <= rsi_val <= 45:
            if not in_downtrend:
                pro_score += 1
        elif 55 <= rsi_val <= 70:
            if not in_uptrend:
                pro_score -= 1

        # MA alignment
        if price > ma20_val > ma50_val:
            pro_score += 2
        elif price < ma20_val < ma50_val:
            pro_score -= 2
        elif in_downtrend:
            pro_score -= 1
        elif in_uptrend:
            pro_score += 1

        # BB (trend-aware)
        if price <= bb_lower:
            pro_score += -1 if in_downtrend else 2
        elif price >= bb_upper:
            pro_score += 1 if in_uptrend else -2

        # Volume
        if vol_avg20 > 0:
            vol_ratio = vol_avg5 / vol_avg20
            if vol_ratio > 1.5:
                pro_score += 2 if direction == "UP" else -2
            elif vol_ratio > 1.2:
                pro_score += 1 if direction == "UP" else -1

        # OBV
        if obv_rising and direction == "UP":
            pro_score += 1
        elif not obv_rising and direction == "DOWN":
            pro_score -= 1

        pro_score = max(-10, min(10, pro_score))

        if pro_score >= 5:
            rec = "STRONG BUY"
        elif pro_score >= 2:
            rec = "BUY"
        elif pro_score <= -5:
            rec = "STRONG SELL"
        elif pro_score <= -2:
            rec = "SELL"
        else:
            rec = "HOLD"

        # Swing targets
        target_up = round(price + atr_val * 2, 2)
        target_down = round(price - atr_val * 2, 2)
        sl_long = round(price - atr_val, 2)
        sl_short = round(price + atr_val, 2)

        return {
            "stock": symbol.upper(),
            "price": round(price, 2),
            "pro_score": pro_score,
            "recommendation": rec,
            "rsi": round(rsi_val, 2),
            "direction": direction,
            "confidence": round(prob * 100),
            "target_up": target_up,
            "target_down": target_down,
            "sl_long": sl_long,
            "sl_short": sl_short,
        }
    except Exception as e:
        log.warning(f"Error analyzing {symbol}: {e}")
        return None


def send_telegram(message):
    """Send a message via Telegram Bot API."""
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "Markdown"}
    resp = requests.post(url, json=payload, timeout=15)
    result = resp.json()
    if not result.get("ok"):
        log.error(f"Telegram error: {result.get('description', 'Unknown')}")
        return False
    return True


def main():
    log.info("=== Auto Alert started ===")

    # Skip weekends
    today = datetime.now()
    if today.weekday() >= 5:
        log.info("Weekend — skipping.")
        return

    model = load_model()
    if model is None:
        log.error("Model not found. Train the model first via the app.")
        send_telegram("⚠️ Stock Alert FAILED: Model not trained. Open the app and train the model.")
        return

    log.info(f"Scanning {len(SCANNER_STOCKS)} stocks (threshold={SCORE_THRESHOLD}, period={SCAN_PERIOD})...")

    buy_picks = []
    sell_picks = []

    for sym in SCANNER_STOCKS:
        res = analyze_stock(sym, model, SCAN_PERIOD)
        if res and abs(res["pro_score"]) >= SCORE_THRESHOLD:
            if res["pro_score"] > 0:
                buy_picks.append(res)
            else:
                sell_picks.append(res)

    buy_picks.sort(key=lambda x: x["pro_score"], reverse=True)
    sell_picks.sort(key=lambda x: x["pro_score"])

    if not buy_picks and not sell_picks:
        log.info("No high-conviction signals found.")
        send_telegram(
            f"📊 *Morning Scan — {today.strftime('%d-%b-%Y')}*\n"
            f"No stocks with |score| ≥ {SCORE_THRESHOLD} today.\n"
            f"_Market may be range-bound. Check app for details._"
        )
        return

    # Build message
    lines = []
    lines.append(f"📈 *Morning Stock Alert — {today.strftime('%d-%b-%Y %I:%M %p')}*")
    lines.append(f"Threshold: |score| ≥ {SCORE_THRESHOLD}  |  Period: {SCAN_PERIOD}\n")

    if buy_picks:
        lines.append("🟢 *BUY PICKS:*")
        for s in buy_picks:
            lines.append(
                f"  *{s['stock']}*  ₹{s['price']}\n"
                f"  Score: *{s['pro_score']:+d}/10*  {s['recommendation']}\n"
                f"  RSI: {s['rsi']}  Conf: {s['confidence']}%\n"
                f"  Target: ₹{s['target_up']}  SL: ₹{s['sl_long']}\n"
            )

    if sell_picks:
        lines.append("🔴 *SELL PICKS:*")
        for s in sell_picks:
            lines.append(
                f"  *{s['stock']}*  ₹{s['price']}\n"
                f"  Score: *{s['pro_score']:+d}/10*  {s['recommendation']}\n"
                f"  RSI: {s['rsi']}  Conf: {s['confidence']}%\n"
                f"  Target: ₹{s['target_down']}  SL: ₹{s['sl_short']}\n"
            )

    lines.append(f"_Total: {len(buy_picks)} buys + {len(sell_picks)} sells_")
    lines.append("_Sent by AI Stock Analyzer ❤️_")

    message = "\n".join(lines)

    # Telegram has a 4096 char limit — split if needed
    if len(message) <= 4096:
        ok = send_telegram(message)
    else:
        # Send in chunks
        ok = True
        for i in range(0, len(message), 4000):
            chunk = message[i:i+4000]
            if not send_telegram(chunk):
                ok = False

    if ok:
        log.info(f"Alert sent: {len(buy_picks)} buys, {len(sell_picks)} sells")
    else:
        log.error("Failed to send Telegram alert")

    log.info("=== Auto Alert finished ===")


if __name__ == "__main__":
    main()
